...
class UsuarioDAO {
  ...
  static async autenticar(nombre, password) {
    ...
    await this.actualizarUltimaSesion(usuario.usuario_id);
  ...
  static async actualizarUltimaSesion(id) {
    const sql = `UPDATE usuarios SET ultima_sesion = NOW() WHERE usuario_id = ?`;
    await pool.query(sql, [id]);
  }

  static async listarTodos() {
    const sql = `SELECT usuario_id as id, nombre, rol, ultima_sesion, activo FROM usuarios ORDER BY usuario_id`;
  ...
  static async desactivar(id) {
    const sql = `UPDATE usuarios SET activo = false WHERE usuario_id = ?`;
    await pool.query(sql, [id]);
  }
  ...
  static async listarOperadores() {
    const sql = `SELECT usuario_id as id, nombre FROM usuarios WHERE rol = 'OPERADOR' AND activo = true`;
    const [rows] = await pool.query(sql);
    return rows;
  }
}
