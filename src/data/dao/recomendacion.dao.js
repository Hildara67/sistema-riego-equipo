...
class RecomendacionDAO {
  ...
  static async obtenerPendientes() {
    const sql = `SELECT r.*, p.nombre as nombre_parcela, p.cultivo 
      ...
      JOIN parcelas p ON r.parcela_id = p.parcela_id 
  ...
  static async obtenerTodas() {
    const sql = `SELECT r.recomendacion_id as id, r.parcela_id, r.volumen_sugerido_L, r.accion, r.estado, r.urgencia, r.usuario_aprobador, r.timestamp_generacion, p.nombre as nombre_parcela, p.cultivo, u.nombre as operador, u.rol as operador_rol
      FROM recomendaciones_riego r 
      JOIN parcelas p ON r.parcela_id = p.parcela_id 
      LEFT JOIN usuarios u ON r.usuario_aprobador = u.usuario_id
      ORDER BY r.timestamp_generacion DESC`;
    const [rows] = await pool.query(sql);
    return rows;
  }
  ...
  static async enviarAValidacion(id, usuarioOperador) {
    const sql = `UPDATE recomendaciones_riego SET usuario_aprobador = ? WHERE recomendacion_id = ?`;
  ...
  static async obtenerPorId(id) {
    const sql = `SELECT * FROM recomendaciones_riego WHERE recomendacion_id = ?`;
